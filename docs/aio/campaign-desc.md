...
Для фильтра по тегу команды, необходимо нажать на кнопку `Теги` и в всплывающем окне выбрать тег источника `GGL` или `FB`
...
В левой панели указаны все детали кампании по типу названия, описания, страна для которой сетап, все метки и т.д.
...
!!! warning "Что необходимо делать, когда домен забанен" 
    В таком случае необходимо поменять цвет во втором пункте на *красный* и назначить дополнительный тег `Banned`. Кампании с такими параметрами будут удаляться для сохранения порядка в трекере.
...
2. `Mode` - это тип клоаки, который сейчас выставлен
    1. `Filter` - это стандартный режим клоаки для залива. Целевая аудитории идёт на `Black`, Нецелевая аудитории идёт на `White`.
...
!!! warning "Важно!"
    Настройки `Flow` в правой части окна кампании трогать нельзя.
